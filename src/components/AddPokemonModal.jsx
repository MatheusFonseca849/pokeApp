...
const AddPokemonModal = ({ visible, onDismiss, teamId, teamColor }) => {
  const { searchPokemon, searchText, unfilteredPokeList, pokeList, next } =
    useContext(PokemonContext);

  const { addPokemonToTeam, removePokemonFromTeam, teams } =
  ...
  const renderFooter = () => {
    ...
    return (
      <View style={styles.loaderContainer}>
        <ActivityIndicator
          size="small"
          color={teamColor || theme.colors.primary}
        />
      </View>
    );
  };
        ...
        <Dialog.Content>
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              const isPokemonInTeam =
                (teamPokemon &&
                  teamPokemon.some((pokemon) => pokemon.url === item.url)) ||
              ...
              return (
                <Card
                  style={[
                    styles.pokemonItem,
                    {
                      borderLeftWidth: isPokemonInTeam ? 8 : 0,
